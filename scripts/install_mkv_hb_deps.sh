...
# makemkv deps
apt-get install -yq --no-install-recommends openjdk-11-jre-headless
apt-get install -yq --no-install-recommends ca-certificates g++ gcc gnupg dirmngr libavcodec-dev libexpat-dev libssl-dev make pkg-config qtbase5-dev wget zlib1g-dev
...
if [[ $(dpkg --print-architecture) =~ arm.* ]]; then
    ...
    apt-get install -yqq handbrake-cli
